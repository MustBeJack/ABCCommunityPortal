...
public function searchUser($firstname, $lastname){
            return UserManagerDB::searchUser($firstname,$lastname);
    }
	...
	public function randomPassword($length,$count, $characters) 
	{
		...
		for ($p = 0; $p < $count; $p++) {
			$pass = '';
			for ($i = 0; $i < $length; $i++) {
				$n = rand(1, $symbols_length); // get a random character from the string with all characters
