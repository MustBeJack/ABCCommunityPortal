...
class UserManagerDB
{
    ...
    public static function searchUser($firstname,$lastname) {
        $users[]=array();
        $conn=DBUtil::getConnection();
        $sql= "select * from tb_user where firstname like '%{$firstname}%' or lastname like '%{$lastname}%' ";
//         echo $sql;
        $result = $conn->query($sql);
        if ($result->num_rows > 0) {
...
// $db=mysql_select_db('phpcrudsample');
